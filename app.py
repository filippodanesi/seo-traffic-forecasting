...
def load_ga4_data(uploaded_file):
    ...
    try:
            ...
            if data_start > 0:
                try:
                    ...
                    try:
                        ...
                        st.info(f"Loaded as semicolon-delimited data")
                    except:
                        try:
                            ...
                            try:
                                ...
                                # If all else fails, try to split the columns manually
                                try:
                                    ...
                                    if ',' in header_line:
                                        ...
                                        st.info("Parsed CSV manually by splitting on commas")
                                    else:
                                        raise ValueError("Could not determine CSV format")
                                except:
                                    raise ValueError(f"Failed to parse file after skipping metadata: {e}")
                except Exception as e:
                    raise ValueError(f"Failed to parse CSV with any delimiter: {e}")
            else:
        ...
        # Clean up column names if they contain delimiters
        if len(df.columns) == 1 and any(delim in df.columns[0] for delim in [',', ';']):
            ...
            # Check if we have data that needs to be split
            if len(df) > 0 and isinstance(df.iloc[0, 0], str):
                data = []
                delimiter = ',' if ',' in df.iloc[0, 0] else ';' if ';' in df.iloc[0, 0] else None
                
                if delimiter:
                    for _, row in df.iterrows():
                        values = [v.strip() for v in row[0].split(delimiter)]
                        if len(values) == len(new_cols):
                            data.append(values)
                            
                    if data:
                        df = pd.DataFrame(data, columns=new_cols)
                        st.info(f"Successfully split data into columns: {new_cols}")
        ...
        date_columns = []
        date_terms = ['date', 'day', 'week', 'month', 'year', 'time', 'settimana', 'data', 
                     'giorno', 'mese', 'anno', 'período', 'semana', 'fecha', 'jour', 'semaine', 'datum']
                     
        for col in df.columns:
            col_lower = col.lower()
            # Check for date-related names in multiple languages
            if any(date_term in col_lower for date_term in date_terms):
        ...
        metric_col = None
        metric_terms = ['user', 'visit', 'session', 'traffic', 'view', 'utent', 'utenti', 
                       'visitator', 'visitante', 'utilisateur', 'benutzer', 'visitas', 
                       'tráfico', 'traffico', 'trafic', 'visualizzazioni', 'vistas', 'vues']
        for col in df.columns:
...
def prepare_analytics_data(df):
    ...
    and using the first day of the month as the date.
    """
    # Rimuovi eventuali duplicati prima dell'aggregazione
    df = df.drop_duplicates()
    
    # Converti a periodo mensile
    df['Month'] = df['Date'].dt.to_period('M')
    ...
    # Converti il periodo a datetime (primo giorno del mese)
    monthly_data['Date'] = monthly_data['Month'].apply(lambda r: r.to_timestamp(how='S'))
    monthly_data.drop(columns='Month', inplace=True)
    
    # Riordina e rimuovi eventuali duplicati
    monthly_data = monthly_data.sort_values('Date').drop_duplicates()
...
def calculate_ga4_forecast(df, forecast_months=24):
    ...
    try:
        # Verifica e rimuovi eventuali duplicati per data
        df = df.drop_duplicates(subset=['Date'])
        
        # Verifica righe per debug
        st.write(f"Righe prima del forecast: {len(df)}")
        ...
        # Verifica righe dopo forecast
        st.write(f"Righe dati storici: {len(historical_df)}, Righe forecast: {len(forecast_df)}")
...
def main():
    ...
    if data_source == "Google Search Console":
        ...
        **How to export data from Google Search Console:**
        ...
        st.info("""
        **Come esportare dati da Google Analytics 4:**
        1. Accedi a Google Analytics 4
        2. Vai su "Reports" > "Acquisition" > "Acquisition Overview"
        3. Imposta il filtro temporale sugli **ultimi 12 mesi** 
        4. Clicca sul pulsante "Condividi questo report" in alto a destra
        5. Seleziona "Scarica file"
        
        ⚠️ **Importante**: 
        - Esporta i dati con colonne "Data" e "Utenti totali"
        - Il formato del file deve essere CSV
        - Almeno 8 settimane di dati sono raccomandate per previsioni accurate
        """)
        ...
        if data_source == "Google Search Console":
            ...
            if df is not None:
                ...
                if st.session_state.get("debug_mode", False):
                    st.write("**Debug: GSC Data after preparation**")
                ...
                if st.button("Generate forecast"):
                    ...
                    if historical_df is not None and forecast_df is not None:
                        ...
                        if st.session_state.get("debug_mode", False):
                            st.write("**Debug: Forecast Data**")
                            st.write(f"Historical shape: {historical_df.shape}")
                            st.write(f"Forecast shape: {forecast_df.shape}")
            ...
            if df is not None:
                # Verifica che non ci siano duplicati già dall'inizio
                initial_len = len(df)
                df = df.drop_duplicates()
                if len(df) < initial_len and st.session_state.get("debug_mode", False):
                    st.warning(f"Rimosse {initial_len - len(df)} righe duplicate dai dati originali")
                
                df = prepare_analytics_data(df)
                
                if st.session_state.get("debug_mode", False):
                    st.write("**Debug: GA4 Data after preparation**")
                    st.write(f"Shape: {df.shape}")
                    st.write(df.head())
                    st.write("Date range:", df['Date'].min(), "to", df['Date'].max())
...
if __name__ == "__main__":
    main(), val) for val in sample_str):
                            if all(pd.to_numeric(val, errors='coerce') < 53 for val in sample_str):  # Max weeks in a year
                                date_columns.append(col)
                    except:
                        continue
